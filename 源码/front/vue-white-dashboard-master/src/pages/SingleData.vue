<template>
  <div class="content">
    ...
    <el-tabs class="table-tab" v-model="activeName">
      <el-tab-pane label="中文简历" name="first">
        <div class="userInfo">
          ...
          <div class="lists">
            <ul>
              <li><span class="label">姓名: </span>&nbsp;{{ userName }}</li>
              <li><span class="label">手机: </span>&nbsp;{{ number }}</li>
              <li><span class="label">籍贯: </span>&nbsp;{{ native }}</li>
              <li><span class="label">专业:</span>&nbsp;{{ speciality }}</li>
              <li><span class="label">工作时长:</span>&nbsp;{{ Working }}</li>
              <li><span class="label">政治面貌:</span>&nbsp;{{ outlook }}</li>
            </ul>
            <ul style="margin-left: 300px">
              <li><span class="label">年龄: </span>&nbsp;{{ year }}</li>
              <li><span class="label">邮箱: </span>&nbsp;{{ emit }}</li>
              <li><span class="label">性别: </span>&nbsp;{{ gender }}</li>
              <li><span class="label">学历:</span>&nbsp;{{ education }}</li>
              <li>
                <span class="label">毕业院校:</span>&nbsp;{{ institutions }}
              </li>
              <li>
                <span class="label">毕业院校类别:</span>&nbsp;{{ category }}
              </li>
            </ul>
          </div>
        </div>
        <div>
          ...
          <div class="lists">
            <ul>
              <li>
                <span class="label">开始时间: </span>&nbsp;{{ startDate }}
              </li>
              <li><span class="label">结束时间: </span>&nbsp;{{ ennDate }}</li>
              <li><span class="label">学位:</span>&nbsp;{{ education }}</li>
              <li><span class="label">专业:</span>&nbsp;{{ speciality }}</li>
              <li><span class="label">学校级别:</span>&nbsp;{{ category }}</li>
            </ul>
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div class="lists">
            <ul>
              <li><span class="label">普通话水平: </span>&nbsp;{{ pth }}</li>
              <li>
                <span class="label">英语水平: </span
                >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ elevel }}
              </li>
              <li><span class="label">编程语言: </span>&nbsp;{{ comcap }}</li>
              <li>
                <span class="label">平面设计能力:</span>&nbsp;{{ designcap }}
              </li>
              <li>
                <span class="label">办公软件掌握:</span>&nbsp;{{ mastercap }}
              </li>
              <li><span class="label">剪辑能力:</span>&nbsp;{{ prcap }}</li>
              <li>
                <span class="label">沟通\逻辑思维\执行\AI能力:</span>&nbsp;{{
                  othercap
                }}
              </li>
            </ul>
      ...
      <el-tab-pane label="候选人画像" name="second">
        ...
        <div class="Highlights">
          <div class="titLabel">
            ...
            <p style="color: black">简历亮点x{{bqlength}}</p>
          </div>
          <ul>
        ...
        <div style="margin-left: 10px">
          ...
          <div class="labels">
            <el-tooltip
              v-for="item in tooltipList3"
              :content="item.content"
              placement="top"
              :key="item.id"
            >
              <el-button
                style="background: #5bda92; border-color: #5bda92"
                type="success"
                round
        ...
        <div
          ref="chart2"
          style="width: 600px; height: 600px; margin-left: 35%"
        ></div>
      </el-tab-pane>
...
export default {
  ...
  data() {
    return {
      ...
      cat: "20 N",
      course:
        "N 国际贸易理论与实务，国际经济与贸易，政治经济学，西方经济学，计量经济学，国际经济学，国际结算，国际金融",
  ...
  created() {
    //this.detail = JSON.stringify(this.$route.params.detail);
    //this.detail = window.localStorage.getItem("detail");
    const storedDetail = localStorage.getItem('detail');
    if (storedDetail) {
        this.detail = JSON.parse(storedDetail);
    }
  },
  mounted() {
    this.fetchData();
  ...
  methods: {
    ...
    // },
    fetchData() {
      console.log("this.datail", this.detail);
      axios
        .post("http://localhost:8181/api/resume/classifyResume", this.detail, {
          headers: {
        ...
        .then((response) => {
          ...
          return axios
            .get("http://localhost:8181/api/resume/selectBasicById", {
              params: {
            ...
            .then((response) => {
              ...
              this.txt = tar1.text;
              return axios
                .get("http://localhost:8181/api/resume/selectEducationalById", {
                  params: {
                    id: parseInt(this.id, 10),
                  },
                })
                .then((response) => {
                  const tar2 = response.data.data;
                  ...
                  this.tooltipList2 = [
                    ...
                    {
                      label: `${tar2.schoolLevel}工程院校`,
                      content: "学校等级标签",
                    },
                  ];
                  return axios
                    .get("http://localhost:8181/api/resume/selectWorkById", {
                      params: {
                        id: parseInt(this.id, 10),
                      },
                    })
                    .then((response) => {
                      const tar3 = response.data.data;
                      const value1 = [];
                      ...
                      return axios
                        .get(
                          "http://localhost:8181/api/resume/selectSkillById",
                          {
                            params: {
                              id: parseInt(this.id, 10),
                            },
                        ...
                        .then((response) => {
                          ...
                          return axios
                            .post(
                              "http://localhost:8181/api/resume/lightResume",
                              this.detail,
                              {
                                headers: {
                                  "Content-Type":
